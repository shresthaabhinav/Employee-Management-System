      ...
      sm:p-12 lg:p-16 relative overflow-y-auto min-h-screen">
        ...
        <div className="space-y-4">
            {portalOptions.map((portal)=>(
                ...
                p-5 sm:p-6 transition-all duration-300 hover:border-green-400 hover:bg-green-50">
                <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                    <h3 className="text-lg text-slate-800 group-hover:text-green-600 mb-1 transition-colors">{portal.title}</h3>
                    <ArrowRightIcon className="w-4 h-4 text-slate-400 group-hover:text-green-600 group-hover:translate-x-1 transition-all duration-300"/>
                </div>
                </Link>
            ))}
        </div>
