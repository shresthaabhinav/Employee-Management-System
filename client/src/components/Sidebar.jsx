...
const Sidebar = () => {
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        ...
        navItems.map((item)=>{
            ...
            return(
                ...
                font-medium transition-all duration-150 relative ${isActive ? "bg-green-500/12 text-green-300" :
                ...
                <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-green-300" : 
                "text-slate-400 group-hover:text-slate-300"}`}/>
                <span className="flex-1">{item.name}</span>
                {isActive && <ChevronRightIcon className="w-3.5 h-3.5 text-green-500/50"/>}
                </Link>
            )
        })
         )}
  ...
  return (
    <>
      ...
      {/* Sidebar Desktop */}
      <aside
        className="hidden lg:flex flex-col h-full w-[260px] bg-slate-800 text-white shrink-0 border-r border-white/4"
      >
        {sidebarContent}
      </aside>
      ...
      {/* Sidebar Mobile */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-slate-800 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </aside>
    </>
